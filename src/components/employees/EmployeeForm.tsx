...
import { z } from 'zod';
import { HiOutlinePlus, HiOutlineTrash, HiOutlineAcademicCap, HiOutlineCreditCard, HiOutlineDocumentText, HiOutlineUser, HiOutlineBriefcase } from 'react-icons/hi';
...
export default function EmployeeForm({ isOpen, onClose, employee }: EmployeeFormProps) {
    ...
    const [activeSection, setActiveSection] = useState<'info' | 'contract' | 'bank' | 'qualifications' | 'enterprise'>('info');
            ...
            <form onSubmit={handleSubmit(onSubmit as never)} className="space-y-6">
                ...
                <div className="flex gap-2 border-b border-gray-200 dark:border-dark-700 pb-4 overflow-x-auto">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeSection === 'enterprise'
                            ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400'
                            : 'text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-dark-700'
                            }`}
                    >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input
                                ...
                                placeholder="Número Ãšnico de Identificação Tributária"
                            />
                            <Input
                        ...
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    Adicione as formaces acadêmicas e certificaces do colaborador
                                </p>
                            </div>
                ...
                {activeSection === 'enterprise' && (
                    <div className="space-y-6">
                        <Card padding="md" variant="glass" className="border-primary-100 dark:border-primary-900/30">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="p-2 bg-primary-100 dark:bg-primary-900/30 rounded-lg text-primary-600 dark:text-primary-400">
                                    <HiOutlineBriefcase className="w-5 h-5" />
                                </div>
                                <div>
                                    <h4 className="font-semibold text-gray-900 dark:text-white">Configurações Profissionais</h4>
                                    <p className="text-xs text-gray-500">Defina comissões e hierarquia da equipa</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Input
                                    label="Taxa de Comissão Base (%)"
                                    type="number"
                                    step="0.1"
                                ...
                                />
                                <Select
                                    label="Reporta a (Superior Hierárquico)"
                                    {...register('reportsToId')}
                                    options={[
                                        { value: '', label: 'Ninguém (Diretor/Admin)' },
                                        ...(employees || [])
                                            .filter(e => e.id !== employee?.id && (e.role === 'admin' || e.role === 'manager'))
                                            .map(e => ({ value: e.id, label: `${e.name} (${e.role})` }))
                                    ]}
                                    error={errors.reportsToId?.message}
                                />
                            </div>

                            <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                                <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                                    <strong>Nota Professional:</strong> Regras de comissão mais complexas (escalonadas ou por categoria) 
                                    podem ser configuradas no Gestãor de Bónus do módulo comercial.
                                </p>
                            </div>
                        </Card>

                        <div className="space-y-4">
                           <h4 className="font-medium text-gray-900 dark:text-white">KPIs e Indicadores</h4>
                           <div className="grid grid-cols-3 gap-4">
                               <div className="p-4 rounded-lg bg-gray-50 dark:bg-dark-800 text-center">
                                   <p className="text-xs text-gray-500 mb-1">Score de Performance</p>
                                   <p className="text-xl font-bold text-gray-900 dark:text-white">
                                       {employee?.performanceScore || 0}%
                                   </p>
                               </div>
                               <div className="p-4 rounded-lg bg-gray-50 dark:bg-dark-800 text-center">
                                   <p className="text-xs text-gray-500 mb-1">Dias de Férias Restantes</p>
                                   <p className="text-xl font-bold text-gray-900 dark:text-white">
                                       {(employee?.vacationDaysTotal || 22) - (employee?.vacationDaysUsed || 0)}
                                   </p>
                               </div>
                               <div className="p-4 rounded-lg bg-gray-50 dark:bg-dark-800 text-center">
