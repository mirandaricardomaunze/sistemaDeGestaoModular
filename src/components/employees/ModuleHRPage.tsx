    ...
    return (
        <div className="space-y-6 animate-fade-in pb-8">
            ...
            <ConfirmationModal
                ...
                title="Processar Lote de Salários"
                message={`Deseja processar todos os rascunhos de salários para o departamento "${config.moduleName}" no período de ${MONTHS[month - 1]} / ${year}?`}
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2"><HiOutlineSun className={`text-${c}-500`} />Gestor de Férias & Ausências</h2>
            ...
            <Card variant="glass" padding="md">
                ...
                {rules.length === 0 ? (
                    <div className="text-center py-12 text-gray-400">
                        ...
                        <p className="text-sm italic">{t('logistics_module.hr.config.noRules', { defaultValue: 'Nenhuma regra de comissão configurada.' })}</p>
                        <p className="text-xs mt-1">{t('logistics_module.hr.config.setupGlobal', { defaultValue: 'Configure regras globais em RH → Configurações.' })}</p>
                    </div>
                ) : rules.map((rule, i) => (
...
export function ModuleHRPage({ config }: { config: ModuleHRConfig }) {
    ...
    const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
        { id: 'dashboard', label: t('logistics_module.hr.tabs.dashboard', { defaultValue: 'Dashboard' }), icon: <HiOutlineChartBar className="w-5 h-5" /> },
        { id: 'staff', label: t('logistics_module.hr.tabs.staff', { defaultValue: 'Equipa' }), icon: <HiOutlineUsers className="w-5 h-5" /> },
        { id: 'attendance', label: t('logistics_module.hr.tabs.attendance', { defaultValue: 'Assiduidade' }), icon: <HiOutlineClock className="w-5 h-5" /> },
        { id: 'payroll', label: t('logistics_module.hr.tabs.payroll', { defaultValue: 'Processamento' }), icon: <HiOutlineBanknotes className="w-5 h-5" /> },
        { id: 'vacations', label: t('logistics_module.hr.tabs.vacations', { defaultValue: 'Férias' }), icon: <HiOutlineSun className="w-5 h-5" /> },
        { id: 'compliance', label: t('logistics_module.hr.tabs.compliance', { defaultValue: 'Conformidade' }), icon: <HiOutlineShieldCheck className="w-5 h-5" /> },
        { id: 'config', label: t('logistics_module.hr.tabs.config.title', { defaultValue: 'Configurações' }), icon: <HiOutlineCurrencyDollar className="w-5 h-5" /> },
    ];
