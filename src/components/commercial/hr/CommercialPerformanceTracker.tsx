...
export function CommercialPerformanceTracker() {
    ...
    const chartData = targets.map((t: SalesTarget) => ({
        name: t.employee?.name || 'Global',
        ventas: t.current || 0,
        meta: Number(t.value),
        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mt-1">Métricas individuais e ranking de vendas em tempo real</p>
                </div>
            ...
            {targets.length === 0 ? (
                <Card variant="glass" className="p-12 text-center flex flex-col items-center border-dashed border-2">
                    ...
                    <p className="text-gray-500 mb-6 max-w-sm">Comece por configurar metas diárias, semanais ou mensais para a equipa comercial.</p>
                    <Button variant="outline" onClick={handleAdd}>Configurar Primeira Meta</Button>
                </Card>
            ) : (
                <>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <Card variant="glass" className="p-6">
                            ...
                            <div className="space-y-4">
                                {[...targets].sort((a, b) => (b.current || 0) - (a.current || 0)).slice(0, 3).map((item, idx) => (
                                    <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-dark-900/50 border border-gray-100 dark:border-dark-700/50 transition-all hover:scale-[1.02] cursor-pointer group" onClick={() => handleEdit(item)}>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400">{formatCurrency(item.current || 0)}</p>
                                        </div>
                            ...
                            <div className="mt-8 p-4 bg-primary-50 dark:bg-primary-900/10 rounded-xl border border-primary-100 dark:border-primary-800/30">
                                ...
                                <p className="text-[11px] text-primary-600 dark:text-primary-300 leading-relaxed font-medium">
                                    O desempenho médio da equipa está em <strong>{Math.round(targets.reduce((acc: number, t: SalesTarget) => acc + (t.progress || 0), 0) / (targets.length || 1))}%</strong>. 
                                    Considere ajustar as metas diárias para otimizar o fluxo de caixa vespertino.
                                </p>
                            </div>
                        </Card>
                    </div>
