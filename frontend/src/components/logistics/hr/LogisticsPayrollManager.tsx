...
import { Card, Button, Select, Badge, Pagination, TableLoadingState, usePagination } from '../../ui';
...
export const LogisticsPayrollManager: React.FC = () => {
    ...
    const { payroll, isLoading, refetch, processPayroll, markAsPaid } = usePayroll({
        month,
        year,
    ...
    const handleUpdateStatus = async (id: string, status: 'processed' | 'paid') => {
        if (status === 'processed') {
            ...
            await markAsPaid(id, 'logistics-hr');
        }
        refetch();
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card variant="glass" className="p-5 bg-gradient-to-br from-indigo-500/10 to-transparent border-l-4 border-indigo-500">
                    ...
                    <h3 className="text-2xl font-black tracking-tighter">
                        {formatCurrency(payroll.reduce((acc, p) => acc + Number(p.totalEarnings || 0), 0))}
                    </h3>
                </Card>
                <Card variant="glass" className="p-5 bg-gradient-to-br from-green-500/10 to-transparent border-l-4 border-green-500">
                    ...
                    <h3 className="text-2xl font-black tracking-tighter text-green-600">
                        {formatCurrency(payroll.filter(p => p.status === 'paid').reduce((acc, p) => acc + Number(p.netSalary || 0), 0))}
                    </h3>
                </Card>
                ...
                <Card variant="glass" className="p-5 bg-gradient-to-br from-purple-500/10 to-transparent border-l-4 border-purple-500">
                    ...
                    <h3 className="text-2xl font-black tracking-tighter text-purple-600">{payroll.length}</h3>
                </Card>
            </div>
                                    ...
                                    <tr key={p.id} className="hover:bg-gray-50/50 dark:hover:bg-dark-700/30 transition-all">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-gray-900 dark:text-white uppercase text-xs">{p.employee?.name || 'Colaborador'}</p>
                                                    <p className="text-[10px] text-gray-400">{p.employee?.code || p.employeeId}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right font-mono text-xs">{formatCurrency(p.baseSalary)}</td>
                                        <td className="px-6 py-4 text-right font-mono text-xs text-green-600 font-bold">+{formatCurrency(p.bonus || 0)}</td>
                                        <td className="px-6 py-4 text-right font-mono text-xs text-red-500">-{formatCurrency(p.totalDeductions || 0)}</td>
