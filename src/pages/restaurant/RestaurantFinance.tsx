...
export default function RestaurantFinance() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [dashboardData, transData] = await Promise.all([
                restaurantAPI.getFinanceDashboard(selectedPeriod),
                restaurantAPI.getTransactions({ 
                    page, 
                    limit, 
                    search, 
            ...
            logger.error('Error fetching restaurant finance data:', error);
    ...
    const incomeCategories = [
        { value: 'Sales_POS', label: 'Vendas Restaurante (POS)' },
        { value: 'Events', label: 'Eventos e Caterings' },
        { value: 'Takeaway', label: 'Take-away / Delivery' },
        { value: 'Tips', label: 'Gorjetas / Service Charge' },
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                selectedPeriod === option.value
                                    ? 'bg-white dark:bg-dark-700 text-rose-600 shadow-sm'
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        >
                            ...
                            <HiOutlineArrowTrendingUp className={cn(
                                'w-6 h-6',
                                selectedType === 'income' ? 'text-rose-600' : 'text-gray-400'
                            )} />
                            <span className={cn(
                                'font-black uppercase text-xs tracking-widest',
                                selectedType === 'income' ? 'text-rose-600' : 'text-gray-500'
                            )}>
                                Receita
                            </span>
                        </label>
