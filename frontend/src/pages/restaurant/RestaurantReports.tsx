...
import {
    HiOutlineArrowPath, HiOutlineDocumentChartBar, HiOutlineCalendar,
    HiOutlineBanknotes, HiOutlineShoppingCart, HiOutlineChartBar,
} from 'react-icons/hi2';
import { HiOutlineCake, HiOutlineArrowDownTray as HiOutlineDocumentArrowDown } from 'react-icons/hi2';
...
type RestaurantSaleItem = {
    name: string;
    quantity: number;
    unitPrice: number;
};

type RestaurantSaleRow = {
    id: string;
    receiptNumber?: string;
    paymentMethod?: string;
    total: number | string;
    createdAt: string | Date;
    table?: { number?: string | number; name?: string };
    items?: RestaurantSaleItem[];
};
...
export default function RestaurantReports() {
    ...
    const topProducts = useMemo(() => {
        ...
        (sales as RestaurantSaleRow[]).forEach((s) => {
        ...
        <div className="space-y-6 p-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <Button variant="ghost" onClick={() => { void refetch(); }} leftIcon={<HiOutlineArrowPath className="w-5 h-5 text-primary-600 dark:text-primary-400" />}>Atualizar</Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                    { label: 'Receita Total', value: formatCurrency(summary.totalRevenue), icon: HiOutlineBanknotes, color: 'red' },
                    ...
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData}>
                                ...
                                <Tooltip formatter={(v) => formatCurrency(Number(v))} contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-10">
                        <HiOutlineDocumentChartBar className="w-10 h-10 text-gray-300 mx-auto mb-3" />
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-dark-700">
                                {(sales as RestaurantSaleRow[]).map((sale) => (
                                    <tr key={sale.id} className="hover:bg-gray-50 dark:hover:bg-dark-700/50 transition-colors">
                                        ...
                                        <td className="py-3 pr-4 font-bold text-red-600">{formatCurrency(Number(sale.total))}</td>
    ...
    function handleExportExcel() {
        ...
        const rows = (sales as RestaurantSaleRow[]).map((s) => [
            s.receiptNumber,
    ...
    function handleExportPDF() {
        ...
        autoTable(doc, {
            ...
            body: (sales as RestaurantSaleRow[]).map((s) => [
                s.receiptNumber ?? '',
                s.table ? `Mesa ${s.table.number}` : 'Balcão',
                (s.paymentMethod ?? '').toUpperCase(),
        ...
        const finalY = (doc.lastAutoTable?.cursor.y ?? 60) + 15;
        doc.setFontSize(12);
